  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="w-2 h-2 rounded-full" style={{ background: "#FF9A00" }} />
  ...
  return (
    <BrowserChrome url="sistema-pmm.marituba.local/contratos">
      <div className="flex items-center justify-between mb-2">
        <span style={{ color: "#FF9A00" }}>contratos / lista</span>
        <span style={{ color: "#64748b" }}>5 / 312</span>
      </div>
      <div style={{ border: "1px solid rgba(255,154,0,0.1)" }}>
        <div
          ...
          style={{
            ...
            borderBottom: "1px solid rgba(255,154,0,0.08)",
          }}
        >
          <span>id</span><span>fornecedor</span><span>valor</span><span>status</span>
        </div>
        {rows.map((r, i) => {
          const color = r.st === "vigente" ? "#22c55e" : r.st === "renovar" ? "#FF9A00" : "#64748b";
          return (
            <div
              key={r.id}
              ...
              style={{
                color: "#cbd5e1",
                borderBottom: i === rows.length - 1 ? "none" : "1px solid rgba(255,154,0,0.05)",
              }}
            >
              <span style={{ color: "#FF9A00" }}>{r.id}</span>
  ...
  return (
    <BrowserChrome url="monitor.atnetto.tech/dashboard">
      <div className="flex items-center justify-between mb-3">
        <span style={{ color: "#FF9A00" }}>infra / overview</span>
        <span style={{ color: "#22c55e" }}>● live · 30s</span>
      </div>
      <div className="grid grid-cols-2 gap-2">
        {servers.map((s) => {
          const stColor = s.st === "up" ? "#22c55e" : "#FF9A00";
          return (
            <div
              key={s.id}
              className="p-2"
              style={{
                background: "rgba(14,12,9,0.6)",
                border: "1px solid rgba(255,154,0,0.1)",
              }}
            >
              ...
              <div className="h-1 mb-1" style={{ background: "rgba(255,154,0,0.1)" }}>
                <div className="h-full" style={{ width: `${s.cpu}%`, background: s.cpu > 70 ? "#FF9A00" : "#22c55e" }} />
              </div>
              <div style={{ color: "#475569" }}>mem</div>
              <div className="h-1" style={{ background: "rgba(255,154,0,0.1)" }}>
                <div className="h-full" style={{ width: `${s.mem}%`, background: s.mem > 70 ? "#FF9A00" : "#22c55e" }} />
              </div>
            </div>
          );
        })}
      </div>
      <div className="mt-2 px-2 py-1" style={{ background: "rgba(255,154,0,0.06)", border: "1px solid rgba(255,154,0,0.2)" }}>
        <span style={{ color: "#FF9A00" }}>⚠ alert</span>{" "}
...
function NexLicense() {
  ...
  return (
    <BrowserChrome url="nexlicense.atnetto.tech/admin">
      <div className="flex items-center justify-between mb-2">
        <span style={{ color: "#FF9A00" }}>licenses / active</span>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                <span style={{ color: "#FF9A00" }}>{l.key}</span>
...
function Sigma() {
  ...
  return (
    <BrowserChrome url="sigma.marituba.local/home">
      <div className="flex items-center justify-between mb-2">
        <span style={{ color: "#FF9A00" }}>módulos integrados</span>
      ...
      <div className="grid grid-cols-4 gap-1.5">
        {mods.map((m, i) => (
          <div
            ...
            style={{
              background: i % 3 === 0 ? "rgba(255,154,0,0.08)" : "rgba(14,12,9,0.6)",
              border: `1px solid rgba(255,154,0,${i % 3 === 0 ? 0.25 : 0.1})`,
              color: i % 3 === 0 ? "#FF9A00" : "#cbd5e1",
            }}
          >
...
function ComControl() {
  ...
  return (
    <BrowserChrome url="comcontrol.local/ativos">
      <div className="flex items-center justify-between mb-2">
        <span style={{ color: "#FF9A00" }}>ativos / inventário</span>
      ...
      <div style={{ border: "1px solid rgba(255,154,0,0.1)" }}>
        ...
        {items.map((it, i) => (
          <div
            ...
            style={{
              color: "#cbd5e1",
              borderBottom: i === items.length - 1 ? "none" : "1px solid rgba(255,154,0,0.05)",
            }}
          >
            <span style={{ color: "#FF9A00" }}>{it.tag}</span>
...
function NetoAI() {
  return (
    <BrowserChrome url="escritorio.neto.ai/triagem">
      <div className="flex items-center justify-between mb-2">
        <span style={{ color: "#FF9A00" }}>triagem · IA</span>
        <span style={{ color: "#22c55e" }}>● modelo: ativo</span>
      </div>
      <div className="space-y-1.5">
        <div className="px-2 py-1.5" style={{ background: "rgba(14,12,9,0.6)", border: "1px solid rgba(255,154,0,0.1)" }}>
          <div style={{ color: "#94a3b8" }}>
            <span style={{ color: "#475569" }}>cliente:</span> &quot;O sistema não está abrindo desde manhã&quot;
          </div>
          <div className="mt-1 flex items-center gap-1.5" style={{ color: "#FF9A00" }}>
            <span>↳ sugestão</span>
